    ...
    def new_or_get(self, request):
        ...
        if qs.count() == 1:  # here it means user just adding cart for the first time
            ...
            cart_obj = Cart.objects.new(
                user=request.user)  # creating new object with according user who already logged in and has many product in the cart
...
# m2m_changed is a signal used save while using many 2 many connection field setup
...
def pre_save_cart_receiver(sender, instance, *args, **kwargs):
    ...
    if instance.sub_total > 0:
        instance.total = Decimal(instance.sub_total) + Decimal(gst)
    else:
        instance.total = 0.00
